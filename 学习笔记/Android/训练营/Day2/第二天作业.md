...
lastmod: 2026-05-04T12:53:59+08:00
description : "创建三个`Activity`文件，分别为`FirstActivity`， `SecondActivity`， `ThirdActivity`。并且在每个`Act..."
image : img/cat.jpg
draft : false
categories : ["Android"]
tags : ["学习笔记", "Android", "训练营", "Day2"]
---
# 第二天作业
...
创建三个`Activity`文件，分别为`FirstActivity`， `SecondActivity`， `ThirdActivity`。并且在每个`Activity`文件中均在生命周期中打出Log信息。三个`Activity`都有一个`Button`， 由`FirstActivity`跳转`SecondActivity`， `SecondActivity`跳转`ThirdActivity`， `ThirdActivity`再跳转为`SecondActivity`。
...
此时Activity的生命周期与Standard模式一样，因为在跳转时，需要跳转的页面均不在任务栈栈顶，所以每次都会创建一个新的Activity并跳转。
