...
description : "- 实现`RecyclerView`的`Activity`， 即`RecyclerViewActivity`"
image : img/cat.jpg
draft : false
categories : ["Android"]
tags : ["学习笔记", "Android", "训练营", "Day4"]
---
# 第四天作业
...
- 实现`RecyclerView`的`Activity`， 即`RecyclerViewActivity`
- 实体类`GameBean`， 用于存储游戏的名称，状态，和图标
- `RecyclerView`的适配器`adapter` : `GameRecycleAdapter`
