...
`self.data = ...` 处应实现数据读取与预处理，常见方式：
- 从文件读取：`pd.read_csv(file).values` 或 `np.load(file)`
- 从 PyTorch tensor：`torch.FloatTensor(data)` 或 `torch.from_numpy(data)`
- 需统一转为 `torch.Tensor`，否则 DataLoader 无法正确处理
